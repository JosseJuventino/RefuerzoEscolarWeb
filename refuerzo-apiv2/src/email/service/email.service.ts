import {
  ...
  export class EmailService {
    ...
    async sendEmail(
      ...
      try {
        this.logger.log(`Iniciando envío de email por usuario: ${userId || 'sistema'}`);
        
        const params: AWS.SES.SendEmailRequest = this.buildSESParams(sendEmailDto);
        
        const result = await this.ses.sendEmail(params).promise();
  
        this.logger.log(`Email enviado exitosamente. MessageID: ${result.MessageId}`);
    ...
    private handleSesError(error: AWS.AWSError): void {
      ...
      if (error.code === 'InvalidParameterValue') {
        throw new BadRequestException(`Error en configuración del correo: ${error.message}`);
      }
      
      if (error.code === 'MessageRejected') {
        throw new BadRequestException(`El mensaje fue rechazado: ${error.message}`);
      }
